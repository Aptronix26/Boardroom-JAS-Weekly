(() => {
  ...
  const item = (r, i, hit) => {
    ...
    const reason = hit
      ? `Revenue added ${money(r.delta)} versus Wk11; ${conversion}. Commercial quality check: ${attach}.${lowBase}`
      : `Revenue lost ${money(r.delta)} versus Wk11; ${conversion}. Commercial quality check: ${attach}.${lowBase}`;
    const action = hit
    ...
    return `<div class="hm-item"><div class="hm-rank">${i + 1}</div><div><div class="hm-title">${clean(r.store)} <small>· ${r.arm}</small></div><div class="hm-signal">Wk12 ${money(r.wk1Revenue)} · ${hit ? '+' : '−'}${money(r.delta)} vs Wk11</div><div class="hm-reason">${reason}</div><div class="hm-action"><b>Action:</b> ${action}</div></div></div>`;
  };
  const section = document.createElement('section');
  section.className = 'hm-section';
  section.innerHTML = `<div class="hm-head"><div><h2>Hits &amp; Misses — Weekly Comparison</h2><p>Stores ranked on absolute revenue contribution or leakage, then explained through conversion and commercial attach quality.</p></div><span class="hm-period">Wk12 vs Wk11</span></div><div class="hm-grid"><div class="hm-panel hm-hit"><div class="hm-panel-title">Hits — gains worth replicating</div><div class="hm-list">${hits.map((r, i) => item(r, i, true)).join('')}</div></div><div class="hm-panel hm-miss"><div class="hm-panel-title">Misses — leakage to recover</div><div class="hm-list">${misses.map((r, i) => item(r, i, false)).join('')}</div></div></div><div class="hm-note"><b>Method:</b> Rankings use absolute Wk12-versus-Wk11 revenue change so low-base percentages cannot dominate. Descriptions use conversion change, 25% loan attach and 20% trade-in benchmarks. These metrics identify likely operating signals; they do not prove causation.</div>`;
  ...
  globalThis.HITS_MISSES_MODEL = { period: 'Wk12 vs Wk11', hits, misses };
})();
